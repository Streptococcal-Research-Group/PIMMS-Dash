...
def create_venn(run_status, thresh_c, slider_c, session_id):
    ...
    control_set = pimms_df.get_data()[((pimms_df.get_data()[NIM_control_col] <= thresh_c) &
                            (pimms_df.get_data()[perc_control_cols[0]] >= slider_c[0]) &
                            (pimms_df.get_data()[perc_control_cols[1]] <= slider_c[1]))]['unique']
    test_set = pimms_df.get_data()[((pimms_df.get_data()[NIM_test_col] <= thresh_c) &
                         (pimms_df.get_data()[perc_test_cols[0]] >= slider_c[0]) &
                         (pimms_df.get_data()[perc_test_cols[1]] <= slider_c[1]))]['unique']
    # Create Venn
    ...
    * **Set B**:
    ...
    """)
    return html.Img(src=venn_img, id='venn-image'), label
